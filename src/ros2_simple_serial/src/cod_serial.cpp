...
public:
  ...
  {
    this->declare_parameter<std::string>("topic_name", "aft_cmd_vel");
    ...
    subscription_ = this->create_subscription<geometry_msgs::msg::Twist>(
      topic_name_, 10, std::bind(&CmdVelSubscriber::topic_callback, this, std::placeholders::_1));

    RCLCPP_INFO(
      this->get_logger(),
      "Serial config: topic=%s, device=%s, baud=%d",
      topic_name_.c_str(), device_path_.c_str(), baud_rate_);
  }
...
private:
  void topic_callback(const geometry_msgs::msg::Twist::SharedPtr msg) const
  {
    ...
    UartTransporter uart(device_path_, baud_rate_);
    uart.open();
    if (uart.isOpen()) {
      ...
      RCLCPP_ERROR(this->get_logger(), "Failed to open UART: %s", uart.errorMessage().c_str());
    }
  }
